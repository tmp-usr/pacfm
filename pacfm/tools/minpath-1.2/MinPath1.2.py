...
path0= os.path.dirname(repr(__file__))

#minpath= os.path.dirname(repr(__file__))
minpath=path0
data_path= os.path.join(minpath , "data")
keggPath0= seedPath0 = mapPath0  = data_path.lstrip('\'')
glpsol0  = os.path.join(minpath,'glpk-4.6/examples/glpsol').lstrip('\'')

#result= namedtuple('Result','acc name test')
#minpath + "/data", minpath + "/data", minpath + "/glpk-4.6/examples/glpsol"
...
class MinPath:
	...
	def WriteReport(self, minpath, reportfile, detailfile):
		...
		#report_file = open(reportfile+'.my', "w")
		if detailfile:
		...
		for p in range(self.pathTot):
			...
			#report_file.write(self.pathList[p]+'\t'+self.pathName[p]+'\t'+pathvalid[p][2]+'\n')                        
